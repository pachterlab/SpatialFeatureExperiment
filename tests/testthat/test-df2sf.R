...
pts_sf <- readRDS(system.file("testdata/pts_sf.rds",
                              package = "SpatialFeatureExperiment"))
...
test_that("Points but with spotDiameter", {
  ...
  pts_sf_dia <- readRDS(system.file("testdata/pts_sf_dia.rds",
                                   package = "SpatialFeatureExperiment"))
...
test_that("sample_id when I need to split the data frame", {
  ...
  multipts_sf <- readRDS(system.file("testdata/multipts_sf.rds",
                                     package = "SpatialFeatureExperiment"))
  sf_use <- sf_use[,names(multipts_sf)]
  expect_equal(sf_use, multipts_sf, ignore_attr = TRUE)

  multipts_df_wrong_sample <- readRDS(system.file("testdata/pts_sf_dia.rds",
                                                  package = "SpatialFeatureExperiment"))
...
test_that("Linestring", {
  ...
  ls_sf <- readRDS(system.file("testdata/ls_sf.rds",
                               package = "SpatialFeatureExperiment"))
  expect_equal(sf_use, ls_sf, ignore_attr = TRUE)
})

test_that("Not enough vertices for the specified geometry", {
  ls_df_singleton <- readRDS(system.file("testdata/ls_df_singleton.rds",
                                         package = "SpatialFeatureExperiment"))
  expect_warning(sf_use <- df2sf(ls_df_singleton, geometryType = "LINESTRING"),
                 "Removed")
  ls_sf_singleton <- readRDS(system.file("testdata/ls_sf_singleton.rds",
                                         package = "SpatialFeatureExperiment"))
  expect_equal(sf_use, ls_sf_singleton, ignore_attr = TRUE)
})

test_that("Multilinestring", {
  multils_df <- readRDS(system.file("testdata/multils_df.rds",
                                    package = "SpatialFeatureExperiment"))
  multils_sf <- readRDS(system.file("testdata/multils_sf.rds",
                                    package = "SpatialFeatureExperiment"))
...
test_that("Polygons", {
  ...
  pol_sf <- readRDS(system.file("testdata/pol_sf.rds",
                                package = "SpatialFeatureExperiment"))
...
test_that("Multipolygons", {
  mpol_df <- readRDS(system.file("testdata/mpol_df.rds",
                                 package = "SpatialFeatureExperiment"))
  mpol_sf <- readRDS(system.file("testdata/mpol_sf.rds",
                                 package = "SpatialFeatureExperiment"))
