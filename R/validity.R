...
.valid_geometry <- function(g, name_show) {
  ...
  if (length(which_invalid)) {
    paste0("The following items (index shown) in ", name_show,
...
.check_geometries <- function(gs, name_show) {
  msgs <- lapply(seq_along(gs), function(i) {
    if (!is(gs[[i]], "sf")) {
      paste0("Item ", i, " in ", name_show, " is ", class(gs[[i]])[1],
             " rather than sf.\n")
    }
  })
  msgs <- unlist(msgs)
  ...
  msgs <- lapply(seq_along(gs), function(i) {
    if (!"sample_id" %in% names(gs[[i]])) {
      ...
      if (any(!samples_seen %in% ids_coldata)) {
        paste0("Samples ", paste(setdiff(samples_seen, ids_coldata),
                                 collapse = ", "),
...
.check_graph_sample_id <- function(x) {
  ...
  if (any(!graph_sample_ids %in% ids_coldata)) {
    paste0("Samples ", paste(setdiff(graph_sample_ids, ids_coldata),
                             collapse = ", "),
...
setValidity2("SpatialFeatureExperiment", function(object) {
  col_msg <- .check_geometries(int_colData(object)$colGeometries,
                               "colGeometries")
  row_msg <- .check_geometries(int_elementMetadata(object)$rowGeometries,
                               "rowGeometries")
  annot_msg <- .check_geometries(int_metadata(object)$annotGeometries,
                                 "annotGeometries")
