...
.valid_geometry <- function(g, name_show) {
    ...
    if (length(which_invalid)) {
        paste0(
            "The following items (index shown) in ", name_show,
            " are invalid:\n", paste0(which_invalid, ": ", v[which_invalid],
                collapse = "\n"
            )
        )
    } else {
...
.check_annotgeometries <- function(x) {
    ...
    msgs <- lapply(seq_along(gs), function(i) {
        if (!"sample_id" %in% names(gs[[i]])) {
            paste0(
                "Item ", i,
                " of annotGeometries does not have column sample_id.\n"
            )
        } else {
            samples_seen <- as.character(unique(gs[[i]]$sample_id))
            if (any(!samples_seen %in% ids_coldata)) {
                paste0(
                    "Samples ", paste(setdiff(samples_seen, ids_coldata),
                        collapse = ", "
                    ),
...
.check_graphs_item <- function(gs, right_length, mar, sample_id) {
    ...
    # 2. colGraphs: Must all have the same length as there are columns in the gene
    # count matrix
    # rowGraphs: Same length as number of rows
    # annotGraphs: Length is not checked but will throw error if length doesn't
    # match that of the annotGeometry of interest.
...
.check_graph_sample_id <- function(x) {
    ...
    if (any(!graph_sample_ids %in% ids_coldata)) {
        paste0(
            "Samples ", paste(setdiff(graph_sample_ids, ids_coldata),
                collapse = ", "
            ),
...
.check_graph_str <- function(x) {
    ...
    if (is(sg, "DataFrame")) {
        ...
        return(paste0(
            "spatialGraphs must be a DataFrame whose columns are sample_ids ",
...
setValidity2("SpatialFeatureExperiment", function(object) {
    col_msg <- .check_geometries(
        int_colData(object)$colGeometries,
        "colGeometries"
    )
    row_msg <- .check_geometries(
        int_elementMetadata(object)$rowGeometries,
        "rowGeometries"
    )
    annot_msg <- .check_geometries(
        int_metadata(object)$annotGeometries,
        "annotGeometries"
    )
