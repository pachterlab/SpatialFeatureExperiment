...
#'   aggregation or \code{sfc}. For multiple samples, the \code{sf} data frame
#'   must have a column \code{sample_id} to indicate which geometry for which
#'   sample. This argument is optional if \code{cellsize} is specified.
#' @param FUN Function to aggregate the numerical columns in \code{colData} and
#'   the gene count matrix. The function must take a numeric vector as input and
#'   return a numeric scalar. Aggregation is not done when aggregating by
...
setMethod("aggregate", "SpatialFeatureExperiment",
          function(x, by = NULL, FUN = sum, sample_id = "all",
              ...
              if (length(sample_id) > 1L && (is(by, "sfc") || !"sample_id" %in% names(by))) {
                  stop("`b`y must be a sf data frame with a column `sample_id`")
              }
              sfes <- split(x, "sample_id")
              sfes <- lapply(sfes, .aggregate_sample,
                             by = by, FUN = FUN,
                             colGeometryName = colGeometryName,
                             rowGeometryName = rowGeometryName, cellsize = cellsize,
...
#'
#' This function splits an SFE object into multiple SFE objects according to a
#' categorical vector (such as sample IDs) or geometries (all cells/spots
#' intersecting with each geometry will become a separate SFE object).
#'
#' @param x An SFE object
#' @param f A character or vector of length 1 or same length as \code{ncol(x)}.
#'   When length 1, it should be the name of a column in `colData(x)`, and if
#'   not found in \code{colData} then a name of an \code{annotGeometry}. It can
#'   also be a \code{sf} data frame or \code{sfc} to split by geometry. Each row
#'   of the \code{sf} data frame or each element in the \code{sfc} will
#'   correspond to a new SFE object. The \code{sf} data frame must have a column
#'   \code{sample_id} when splitting multiple samples.
...
#' @rdname split-SpatialFeatureExperiment
#' @export
setMethod("split", c("SpatialFeatureExperiment", "ANY"),
          function(x, f) {
